exports.BlackCoinCallback = class {
    constructor (callbackSecret) {
        if(!callbackSecret) throw new Error(`BlackCoin Callback: Вы не передали параметр callback_secret.`);
    ...
    on (callback) {
        if(typeof callback !== 'function') throw new Error('BlackCoin Callback: Callback must be a function.');
...
exports.BlackCoin = class {
    constructor (options = {}) {
        const { token, version } = options;

        if(!token) throw new Error('BlackCoin: Вы не передали ключ авторизации (token).');

        this.api = {
            token,
            headers: { Authorization: 'Bearer ' + token },
            url: 'http://black-coin.ru/black_coin/api/',
            version: version || 'v1'
        };
    };
